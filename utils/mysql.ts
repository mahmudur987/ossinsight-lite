...
export async function withConnection<R>(
  uri: string,
  run: (conn: Connection & SqlExecutor) => Promise<R>
): Promise<R> {
  if (!uri) {
...
export interface SqlInterface {
  <T>(templateStringsArray: TemplateStringsArray, ...args: any[]): Promise<
    (T & RowDataPacket)[]
  >;
...
export function getDatabaseUri(
  ...
  if (
    ...
    console.error(
      "TiDB integration was not configured. Check your vercel project config."
    );
    return "";
  }
  const username = readonly
    ? process.env.TIDB_USER.replace(/\.[^.]*$/, ".osslreadonly")
    : process.env.TIDB_USER;
  const password = readonly
    ? process.env.TIDB_PASSWORD + ".osslreadonly"
    : process.env.TIDB_PASSWORD;
  if (database) {
    return `mysql://${username}:${password}@${process.env.TIDB_HOST}:${
      process.env.TIDB_PORT
    }/${
      use || database
    }?timezone=Z&ssl={"rejectUnauthorized":true,"minVersion":"TLSv1.2"}`;
  } else {
    return `mysql://${username}:${password}@${process.env.TIDB_HOST}:${
      process.env.TIDB_PORT
    }${
