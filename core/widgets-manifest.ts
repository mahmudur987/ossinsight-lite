...
type WidgetModuleMeta<P = any> = {
  ...
  getData?: () => Promise<{
    default(server: ServerContext, props: P): Promise<any>;
  }>;
  getMetadata?: () => Promise<{
    default(server: ServerContext, props: P): Promise<Metadata>;
  }>;
...
export type ServerContext = {
  ...
  runSql(
    db: string,
    sql: string
  ): Promise<{ data: any[]; columns: { name: string; type: number }[] }>;
};
...
type WidgetModule<P = any> = {
  ...
  WidgetDetails?: () => Promise<{
    default: ForwardRefExoticComponent<P & HTMLProps<HTMLDivElement>>;
  }>;
  ConfigureComponent?: () => Promise<{
    default: ForwardRefExoticComponent<P & HTMLProps<HTMLDivElement>>;
  }>;
...
export type { Widgets, WidgetModule, WidgetModuleMeta, ResolvedWidgetModule };

// throw new Error(
//   "This file is a slot, will be actually loaded by buildtool/webpack/loaders/widgets-manifest. Check your config."
// );
