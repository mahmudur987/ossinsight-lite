...
console.log(process.env.TIDB_USER);
...
async function main() {
  ...
  try {
    ...
    if (rows.length !== 0) {
      ...
      const max_id = parseInt(
        files[files.length - 1].replace(/^sql\//, "").split("-")[0]
      );
      if (!(rows[0].order_id <= max_id)) {
        console.warn(
          `The migration version maybe too old (db = ${rows[0].order_id}, source = ${max_id}). Update your source code asap.`
        );
        process.exit(0);
      }
    }
  ...
  for (; i < files.length; i++) {
    ...
    const hookFile = fn.replace(/^sql/, "hook").replace(/\.sql/, ".js");
...
const defaultEnv = {
  INITIAL_PASSWORD: hashSync(
    process.env.SITE_INITIAL_PASSWORD || "tidbcloud",
    1
  ),
  ADMIN_DATABASE_NAME:
    process.env.NEXT_PUBLIC_SITE_DATABASE || "ossinsight_lite_admin",
};
